...
# Plots 10 images with their true labels
def plot_images(images, true_classif, pred_classif=None):
    assert len(images) == len(true_classif) == 5
    ...
    for i, plot in enumerate(axes.flat):
        # Plot images
        plot.imshow(images[i, :, :, :], interpolation='nearest')
    ...
    fig.tight_layout()
    fig.savefig('cifar_conv_miss_classifications.png', bbox_inches='tight')
    plt.show()

# Plots 10 images in a 1x10 subplot with their true labels
def plot_sample(images, true_classif):
    ...
    for i, plot in enumerate(axes.flat):
        ...
        # Add label
        xlabel = "{0}".format(true_classif[i])
        plot.set_xlabel(xlabel)
...
def plot_conv_layer(values, name):

    values_min = np.min(values)
    values_max = np.max(values)

    number_of_filters = values.shape[3]
    number_of_grids = math.ceil(math.sqrt(number_of_filters))
...
def print_confusion_matrix(true_classif, pred_classif, number_of_classes):
    ...
    # Show plot
    plt.savefig('cifar_confusion_matrix.png', bbox_inches='tight')
    plt.show()
